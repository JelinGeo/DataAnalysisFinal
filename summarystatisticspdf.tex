...
\item
  ...
  or skewed distributions, as well as spot outlines and the overall
  spread of the data.
\end{itemize}
...
\FunctionTok{hist}\NormalTok{(data, }\AttributeTok{main=}\StringTok{"Figure1.Histogram of Data"}\NormalTok{, }\AttributeTok{col =} \StringTok{"grey"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{summarystatisticspdf_files/figure-pdf/unnamed-chunk-4-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{boxplot}\NormalTok{(data, }\AttributeTok{main=}\StringTok{"Figure2.Boxplot of Data"}\NormalTok{, }\AttributeTok{col=}\StringTok{"grey"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{summarystatisticspdf_files/figure-pdf/unnamed-chunk-5-1.pdf}}
...
The YouTube videos referenced here may assist in understanding the code
chunks presented above. (\citeproc{ref-walker2023gtsummary}{Walker,
2023}) (\citeproc{ref-dre2024gentle}{Videos, 2024})
...
Next, load the Star Wars dataset available online in the dylyr package.
(\citeproc{ref-dplyr2023}{Wickham et al., 2023})
...
\begin{table}
\caption{Table 1. Star Wars Data}\tabularnewline
...
\FunctionTok{table}\NormalTok{(starwars\_selected}\SpecialCharTok{$}\NormalTok{gender)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Proportion table for gender}
\FunctionTok{prop.table}\NormalTok{(}\FunctionTok{table}\NormalTok{(starwars\_selected}\SpecialCharTok{$}\NormalTok{gender))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{starwars }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Figure3.Histogram of Height of Characters in Star Wars"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{summarystatisticspdf_files/figure-pdf/unnamed-chunk-19-1.pdf}}
...
\NormalTok{starwars }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{binwidth =} \DecValTok{8}\NormalTok{, }\AttributeTok{fill =} \StringTok{"navy"}\NormalTok{, }\AttributeTok{color =} \StringTok{"blue"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Figure4.Histogram of Height of Human Characters in Star Wars"}\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Height (cm)"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"Count"}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
...
Wars dataset} might be an interesting watch to understanding the dataset
better.
...
the book Naked Statistics, the author Charles Wheelan highlights key
limitations of statistics, warning that statistical measures can be
...
(\citeproc{ref-wheelan2013naked}{Wheelan, 2013})
...
artificial intelligence.Several key trends and directions can be
identified:

\begin{itemize}
\item
  \textbf{Integration with Advanced Computational Methods}: As datasets
  grow larger and more complex, summary statistics will increasingly be
...
\item
  \textbf{AI-Driven and Automated Summarization}: Artificial
  intelligence, including machine learning and natural language
  processing, is transforming how summary statistics are generated and
  interpreted. AI-driven summarization tools can quickly distill massive
  and complex datasets into actionable insights, improving efficiency,
...
\item
  \textbf{Enhanced Visualization and Exploratory Data Analysis}: The
  role of visualization in summary statistics will continue to grow,
  leveraging advances in computer graphics and interactive tools. This
  enables more intuitive and exploratory ways to understand data
  distributions, trends, and anomalies, moving beyond traditional tables
  and static plots. (\citeproc{ref-potter2010visualizing}{Potter et al.,
  2010})
\item
  \textbf{Addressing Big Data and Complex Structures}: Summary
  statistics will need to adapt to the challenges of big data, including
  handling massive volumes, varied data types, and complex dependencies
  (such as networks or time-evolving data). This will require new
  conceptual frameworks and algorithms capable of summarizing
  information efficiently at scale.(\citeproc{ref-fan2014challenges}{Fan
  et al., 2014})
\item
  \textbf{Ongoing Methodological Innovation}: The field will continue to
  see progress in robust inference, regularization, causal inference,
  and adaptive decision analysis, all of which will influence how
  summary statistics are computed and applied in practice.
  (\citeproc{ref-gelman2024stat50}{Gelman \& Vehtari, 2024})
\end{itemize}

In summary, the future of summary statistics lies in their evolution
from simple descriptive tools to components of sophisticated, automated,
and interactive analytical systems. They will play a foundational role
in making sense of big, complex, and heterogeneous data, driven by
advances in computation, AI, and interdisciplinary collaboration.
